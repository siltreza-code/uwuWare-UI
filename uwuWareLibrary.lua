...
local function keyCheck(x, set)
	return set[x] == true
end

-- Drag update (FIXED bounds + safer math)
local function update(input)
	if not dragObject then return end
	local delta = input.Position - dragStart

	local newX = startPos.X.Offset + delta.X
	local newY = math.max(startPos.Y.Offset + delta.Y, -36)
...
-- Rainbow (OPTIMIZED: no spawn loop)
local chromaColor
local rainbowTime = 5
...
local function createOptionHolder(holderTitle, parent, parentTable, subHolder)
	...
	parentTable.main = library:Create("ImageButton", {
		LayoutOrder = parentTable.position or 0,
	...
	layout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
		...
		parentTable.main.Size = parentTable.open and UDim2.new(0, 230, 0, y + size) or UDim2.new(0, 230, 0, size)
	end)

	parentTable.content = content
	...
	parentTable.SetTitle = function(_, newTitle)
